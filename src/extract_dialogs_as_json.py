        ...
        for _speechId, line in enumerate(textList):
            ...
            if speaker == "" and len(speechEvents) > 0:
                ...
                speechEvents.append(
                    {
                        "speaker": speaker,
                        "speech": [self.cleanSpeech(speech)],
                        "type": speakerClass,
                    }
                )
...
if __name__ == "__main__":
    extractDialogAsJson(
        "data/super_mario_rpg/cleaned",
        "data/super_mario_rpg/super_mario_rpg.json",
        JsonBuilder(),
        "Super Mario RPG",
        1.0,
    )
    ...
    extractDialogAsJson(
        "data/chrono_trigger/cleaned",
        "data/chrono_trigger/chrono_trigger.json",
        JsonBuilder(),
        "Chrono Trigger",
        1.0,
    )
    ...
    extractDialogAsJson(
        "data/ff6/cleaned", "data/ff6/ff6.json", JsonBuilder(), "Final Fantasy 6", 1.0
    )
    extractDialogAsJson(
        "data/ff7/cleaned", "data/ff7/ff7.json", JsonBuilder(), "Final Fantasy 7", 1.0
    )
    extractDialogAsJson(
        "data/ff8/cleaned", "data/ff8/ff8.json", JsonBuilder(), "Final Fantasy 8", 1.0
    )
    extractDialogAsJson(
        "data/ff9/cleaned", "data/ff9/ff9.json", JsonBuilder(), "Final Fantasy 9", 1.0
    )

    extractDialogAsJson(
        "data/sf3s1/cleaned",
        "data/sf3s1/sf3s1.json",
        JsonBuilder(),
