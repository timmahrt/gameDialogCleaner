...
def saveUrl(i, urlTail, fullUrl, outputPath, extractTextFunc):
    pageTextOutputFn = "%03d_%s.html" % (
        i,
        os.path.splitext(urlTail)[0].replace("/", "_"),
    )
    try:
...
def crawlPageForUrlsDontMangleUrls(
    url, outputPath, prefix, extractTextFunc=downloadScriptOnPage
):
    if not os.path.exists(outputPath):
        os.mkdir(outputPath)
